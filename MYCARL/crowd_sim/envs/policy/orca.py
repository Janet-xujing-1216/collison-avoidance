...
class ORCA(Policy):
    ...
    def predict(self, state):
        ...
        """
        velocity 是机器人当前位置指向目标位置的矢量，speed 是这个矢量的长度，即机器人当前的速度。
        pref_vel 是优先速度矢量，指向目标位置的方向，长度为 1，即表明机器人的理想速度为 1
        """
        ...
        self.sim.setAgentPrefVelocity(0, tuple(pref_vel)) # 0表示自身（如果是robot和human就是robot）
        for i, human_state in enumerate(state.human_states):
            # unknown goal position of other humans
            self.sim.setAgentPrefVelocity(i + 1, (0, 0)) 

        self.sim.doStep()
        action = ActionXY(*self.sim.getAgentVelocity(0)) # 获取的模拟器中代理0的速度向量转换为一个 ActionXY 类型的对象

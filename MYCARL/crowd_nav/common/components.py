...
class ATCBasic(nn.Module):
    ...
    def forward(self, input, hx=None):
        ...
        size = input.shape
        input_ = input.view(-1, size[2]) # (batch_size * sequence_length, input_dim) 的2D tensor.
        selector = input_.data.new(input_.shape[0]).byte()
        """
        新创建的selector张量长度为batch_size * sequence_length，并且默认情况下所有元素都设置为False。
        它用于跟踪已超过max_ponder所规定的最大计算步数的输入序列，以便于forward方法可以提前终止这些输入序列的计算
        """
        ponder_times = []
        accum_p = 0
        ...
        if self.act_fixed:
            ...
            for act_step in range(self.max_ponder): # 计算"Halten"状态的累加和来动态计算步数
                hx = self.rnn_cell(input_, hx)  # GRUModel
                hx = hx.view(-1, self.rnn_hidden_dim)
                halten_p = torch.sigmoid(self.ponder_linear(hx))  # halten state (p_t)^n
                # 每个“Halten”状态是在RNN处理隐藏状态后使用sigmoid函数计算的
                accum_p += halten_p
                accum_hx += halten_p * hx
                step_count += 1 # 选择的gru个数 N(t)
                selector = (accum_p < 1 - self.epsilon).data
                if not selector.any():  # selector has no true elements
                    break

            hx = accum_hx / step_count # y_t ^n

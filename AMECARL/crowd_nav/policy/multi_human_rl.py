...
# 创建并操作一个高斯概率密度函数的查找表 norm.pdf概率分布
...
class MultiHumanRL(CADRL):
    def __init__(self):
        ...
        self.use_rate_statistic = True # 是否启用对算法构建过程中的每个步骤的成功率进行统计的功能
        self.statistic_info = [0 for index in range(20)] # 定义了一个list，其中包含20个元素，每个元素的值都是0
    ...
    def predict(self, state):
        ...
        if self.phase == 'train' and probability < self.epsilon:
            ...
            """
            这段代码是在记录算法的局部最优解的出现次数。当use_rate_statistic为True时，会统计每个步骤（single_step_cnt）的出现次数，即记录这个步骤是多少次找到的局部最优解。
            self.statistic_info是一个包含20个元素的列表，用于存储统计信息，每当一个步骤完成一次找到局部最优解的过程，就会将相应的元素加1。
            如果步骤计数值（single_step_cnt）超过了列表长度，就会输出 step count too large!! 的警告
            """
            if self.use_rate_statistic:
    ...
    def compute_reward(self, nav, humans, collision_prob=0.0):
        ...
        if collision:
            ...
            reward = -0.25 * collision_prob * 4.0  # 论文里beta = 2.0
